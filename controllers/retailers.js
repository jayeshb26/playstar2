...
exports.getWinnerResultsByDate = asyncHandler(async (req, res, next) => {
  ...
  // const winnerHistory = await WinResult.find();
  // res
  //   .status(200)
...
exports.claimeTicket = asyncHandler(async (req, res, next) => {
  ...
  let result = "Ticket Id Not Found";
  //console.log(bets, bets.length, bets[0].claim, "  ", req.body.ticketId, " ", req.body.ticketId.toString().toUpperCase(),)
  if (bets.length != 0) {
    if (bets[0].isCancel == true) {
      result = "Ticket Already Cancelled.";
    } else if (bets[0].retailerId == req.user.id) {
      if (bets[0].winPosition == "") result = "Result Not yet Declared";
      else if (bets[0].claim == true) {
        ...
        if (bets[0].won != 0) {
          ...
          //   $inc: { creditPoint: bets[0].won },
          // });
          result = "Congratulations, you have won Rs." + bets[0].won;
        } else result = "No Win";
      }
    } else result = "You purchased your ticket from other retailer..";
  }
...
exports.getCommissionByDate = asyncHandler(async (req, res, next) => {
  //console.log(req.query.dateStart, req.body.dateStart, req.params.dateStart);
  const data = await Bet.aggregate([
    {
      $match: {
        retailerId: mongoose.Types.ObjectId(req.user.id),
